...
# ==============================================================================
# Forgeyard Documentation Build Script (Pure Bash & mdBook)
# Assembles src/ directory and compiles static book with mdBook
# ==============================================================================

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SRC_DIR="${ROOT_DIR}/src"
SYS_ARCH_DIR="${ROOT_DIR}/sys-arch"
DOCS_DIR="${ROOT_DIR}/docs"
WIKI_DIR="${ROOT_DIR}/wiki"

echo "============================================================"
echo "Forgeyard Documentation Build (mdBook)"
echo "Source: ${SRC_DIR}"
echo "============================================================"

# Prepare src/ subdirectories
mkdir -p "${SRC_DIR}/sys-arch"
...
if [ -f "${WIKI_DIR}/Home.md" ]; then
  echo "Generating src/README.md from wiki Home portal..."
  # Replace internal wiki markdown links with mdBook relative paths
  sed -E 's|\]\(([0-9]{2}-[a-z0-9-]+)\)|](sys-arch/\1.md)|g; s|\]\((forgeyard-[a-z0-9-]+)\)|](sys-arch/\1.md)|g' "${WIKI_DIR}/Home.md" > "${SRC_DIR}/README.md"
fi

# Generate src/SUMMARY.md with pure bash
echo "Generating src/SUMMARY.md table of contents..."
cat << 'EOF' > "${SRC_DIR}/SUMMARY.md"
# Summary
...
# Compile with mdBook if installed
if command -v mdbook >/dev/null 2>&1; then
  echo "Compiling documentation book using mdBook..."
  mdbook build "${ROOT_DIR}"
  echo "============================================================"
  echo "SUCCESS: Documentation compiled into ./book/"
  echo "============================================================"
else
  echo "Warning: mdbook command not found in PATH." >&2
fi
